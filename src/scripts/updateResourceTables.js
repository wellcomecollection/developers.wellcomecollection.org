...
const formatBytes = (bytes, sigFigs) => {
    if (bytes == 0) {
        return "0 Bytes";
    }
...
const getResourceData = async (resourceUrl) => {
    ...
    if (resp.ok) {
        ...
        const lastModified = lastModifiedHeader !== null ? new Date(lastModifiedHeader) : undefined;
        return {
            size,
            lastModified
        };
...
const updateResourceTables = () => {
    const dataLinks = document.querySelectorAll("table a[href^='https://data.wellcomecollection.org']");
